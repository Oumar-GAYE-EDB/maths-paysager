...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const modeAideRenforcee = document.getElementById("mode-aide-renforcee");
  ...
  initialiserModeExercices({
    ...
    modeAideRenforcee: modeAideRenforcee,
...
function initialiserModeExercices(ui) {
  ...
  ui.btnGenererExercice.addEventListener("click", function () {
    ...
    if (ui.modeAideRenforcee && ui.modeAideRenforcee.checked) {
      afficherIndiceProgressif(ui.feedbackExercice, exerciceActuel, 0);
    }
    mettreAJourAffichageEtapes(ui, 1);
  ...
  if (ui.modeAideRenforcee) {
    ui.modeAideRenforcee.addEventListener("change", function () {
      const actif = !!ui.modeAideRenforcee.checked;
      if (ui.btnIndice2) ui.btnIndice2.hidden = actif;
      if (ui.btnMethode) ui.btnMethode.hidden = actif;
      if (ui.feedbackExercice && actif && exerciceActuel) {
        afficherIndiceProgressif(ui.feedbackExercice, exerciceActuel, 0);
      }
    });
  }
  ...
  if (ui.modeAideRenforcee && ui.modeAideRenforcee.checked) {
    if (ui.btnIndice2) ui.btnIndice2.hidden = true;
    if (ui.btnMethode) ui.btnMethode.hidden = true;
  }
  ...
  mettreAJourDiagnosticPedagogique(ui.diagnosticExercice, ui.planRevision);
  mettreAJourAffichageEtapes(ui, 0);
}

function mettreAJourAffichageEtapes(ui, etape) {
  const panels = document.querySelectorAll(".step-panel");
  panels.forEach(function (panel) {
    const match = panel.className.match(/step-(\d)/);
    if (!match) return;
    const panelStep = parseInt(match[1], 10);
    panel.hidden = panelStep > etape;
  });
}
...
function corrigerExercice(ui, passerAuSuivant) {
  ...
  if (!passerAuSuivant) {
    ...
    afficherHistorique(ui.historiqueExercice);
    mettreAJourDiagnosticPedagogique(ui.diagnosticExercice, ui.planRevision);
    mettreAJourAffichageEtapes(ui, 2);
    return;
  }
...
function afficherPlanMaitrise(zone) {
  ...
  zone.dataset.status = progression.essais >= 3 ? "warn" : "todo";
...
function afficherChecklistVerification(zone, exercice, estValide) {
  if (!zone || !exercice) return;
  zone.dataset.status = estValide ? "ok" : "todo";
  zone.innerHTML =
...
function afficherPlanRemediation(zone, exercice, diagnostic) {
  if (!zone || !exercice) return;
  zone.dataset.status = diagnostic ? "warn" : "ok";
  ...
  const microCours = erreursConsecutives >= 2
    ...
    : "";
  zone.className = "exercise-panel" + (diagnostic ? " resultat--remediation" : "");
  zone.innerHTML =
